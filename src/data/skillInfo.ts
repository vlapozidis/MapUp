...
export const skillInfo: Record<string, SkillEntry> = {
	...
	'Azure CLI': {
		en: "A command-line tool for creating and managing Azure resources — `az vm create`, `az group list`, and so on — instead of clicking through the Portal.\n\nThe Portal is fine for a first look at something, but the CLI is what you actually script, put in a pipeline, or paste into a runbook. Once you can log in with `az login` and query resources with `--query`, most day-to-day Azure work stops requiring a browser at all.",
		el: 'Ένα εργαλείο command-line για δημιουργία και διαχείριση πόρων Azure — `az vm create`, `az group list` και άλλα — αντί να κάνεις κλικ στο Portal.\n\nΤο Portal είναι μια χαρά για μια πρώτη ματιά, αλλά το CLI είναι αυτό που πραγματικά scriptάρεις, βάζεις σε pipeline ή κολλάς σε runbook. Μόλις μάθεις να συνδέεσαι με `az login` και να κάνεις queries πόρων με `--query`, η καθημερινή δουλειά στο Azure σταματάει να χρειάζεται browser.',
	},
	'Azure Data Studio': {
		en: "A free, cross-platform editor for Azure SQL and SQL Server, built for people who don't want the full weight of SSMS or aren't on Windows.\n\nIt's lighter than SSMS and feels more like a modern code editor — tabs, a command palette, notebooks for mixing SQL and notes. Plenty of DBAs keep both installed: SSMS for the deep admin work, Azure Data Studio for quick queries and anything cross-platform.",
		el: 'Ένας δωρεάν, cross-platform editor για Azure SQL και SQL Server, φτιαγμένος για όσους δεν θέλουν όλο το βάρος του SSMS ή δεν είναι σε Windows.\n\nΕίναι πιο ελαφρύς από το SSMS και μοιάζει περισσότερο με σύγχρονο code editor — tabs, command palette, notebooks για συνδυασμό SQL και σημειώσεων. Πολλοί DBAs έχουν και τα δύο εγκατεστημένα: SSMS για τη βαθιά διαχείριση, Azure Data Studio για γρήγορα queries και ό,τι είναι cross-platform.',
	},
	'Azure DevOps': {
	...
	'Browser DevTools': {
		en: "The Inspector, Console, and Network tabs built into every browser — free, always available, and the fastest way to see what a web page is actually doing.\n\nBefore filing a bug or asking an engineer for help, checking the Console for red errors or the Network tab for a failed request often tells you the answer yourself. It's as useful for a QA engineer reproducing a bug as it is for a support agent trying to describe what actually broke.",
		el: 'Τα tabs Inspector, Console και Network που υπάρχουν σε κάθε browser — δωρεάν, πάντα διαθέσιμα και ο γρηγορότερος τρόπος να δεις τι κάνει πραγματικά μια σελίδα.\n\nΠριν καταχωρήσεις ένα bug ή ζητήσεις βοήθεια από engineer, ένα κοίτα στο Console για κόκκινα errors ή στο Network για ένα αποτυχημένο request συχνά σου δίνει μόνο του την απάντηση. Είναι εξίσου χρήσιμο για έναν QA engineer που αναπαράγει ένα bug όσο και για έναν support agent που προσπαθεί να περιγράψει τι χάλασε.',
	},
	'Bug Lifecycle': {
	...
	'Dialer Software': {
		en: 'The tool that places, logs, and often records sales calls automatically, instead of you dialing numbers by hand and typing up notes afterward.\n\nBeyond saving time, a good dialer syncs straight into the CRM — no call ever "forgets" to get logged, which means your pipeline data stays trustworthy without you having to think about it. Most also queue up your next call automatically, so there’s less dead time between conversations.',
		el: 'Το εργαλείο που κάνει, καταγράφει και συχνά ηχογραφεί τις κλήσεις πωλήσεων αυτόματα, αντί να καλείς αριθμούς με το χέρι και να γράφεις σημειώσεις μετά.\n\nΠέρα από την εξοικονόμηση χρόνου, ένα καλό dialer συγχρονίζεται απευθείας με το CRM — καμία κλήση δεν «ξεχνιέται» να καταγραφεί, οπότε τα δεδομένα του pipeline παραμένουν αξιόπιστα χωρίς να το σκέφτεσαι. Τα περισσότερα βάζουν και την επόμενη κλήση σου αυτόματα στη σειρά, οπότε μένει λιγότερος νεκρός χρόνος ανάμεσα σε συζητήσεις.',
	},
	'Discovery Calls': {
	...
	'Execution Plans': {
		en: 'The step-by-step breakdown of exactly how SQL Server plans to run a query — which indexes it uses, whether it scans or seeks, in what order it joins tables.\n\nTwo queries that look identical can have wildly different plans, and the plan is usually the only way to explain why one is instant and the other times out. Learning to read one — spotting a table scan where an index seek should be — is the single most useful debugging skill for anything query-tuning related.',
		el: 'Η αναλυτική περιγραφή του πώς σκοπεύει ο SQL Server να τρέξει ένα query — ποια indexes χρησιμοποιεί, αν κάνει scan ή seek, με ποια σειρά κάνει joins τους πίνακες.\n\nΔύο queries που φαίνονται ίδια μπορεί να έχουν εντελώς διαφορετικό plan, και το plan είναι συνήθως ο μόνος τρόπος να εξηγήσεις γιατί το ένα είναι στιγμιαίο και το άλλο κάνει timeout. Το να μάθεις να διαβάζεις ένα — να εντοπίζεις ένα table scan εκεί που θα έπρεπε να υπάρχει index seek — είναι η πιο χρήσιμη δεξιότητα debugging για οτιδήποτε σχετίζεται με query tuning.',
	},
	'Exploratory Testing': {
	...
	kubectl: {
		en: "The command-line tool for talking to a Kubernetes cluster — deploying workloads, checking pod status, reading logs, all from the terminal.\n\n`kubectl get pods` and `kubectl logs` are the two commands you'll type more than anything else once containers are actually running somewhere. The Kubernetes dashboard exists too, but almost every real troubleshooting session starts and ends in a terminal with kubectl.",
		el: 'Το εργαλείο command-line για επικοινωνία με ένα Kubernetes cluster — deploy workloads, έλεγχος κατάστασης pods, ανάγνωση logs, όλα από το terminal.\n\nΤο `kubectl get pods` και το `kubectl logs` είναι οι δύο εντολές που θα γράφεις περισσότερο από όλες μόλις τα containers τρέχουν κάπου πραγματικά. Υπάρχει και το Kubernetes dashboard, αλλά σχεδόν κάθε πραγματικό troubleshooting ξεκινάει και τελειώνει σε ένα terminal με kubectl.',
	},
	'Lead Qualification': {
	...
	'Node.js': {
		en: 'A JavaScript runtime that runs outside the browser, which is what lets you use the same language on your backend as you do in the browser.\n\nIt comes bundled with npm, so installing Node.js is also how you get the package manager most JavaScript projects depend on. Almost every local dev environment for a JS-based backend or build tool starts with "install Node.js" as literally step one.',
		el: 'Ένα JavaScript runtime που τρέχει έξω από τον browser — αυτό σου επιτρέπει να χρησιμοποιείς την ίδια γλώσσα και στο backend και στον browser.\n\nΈρχεται μαζί με το npm, οπότε η εγκατάσταση του Node.js σου δίνει και τον package manager από τον οποίο εξαρτώνται τα περισσότερα JavaScript projects. Σχεδόν κάθε τοπικό dev environment για JS backend ή build tool ξεκινά με «εγκατάστησε το Node.js» ως κυριολεκτικά πρώτο βήμα.',
	},
	'Non-relational Data': {
	...
	'Remote Access Tools': {
		en: "VPN clients, remote desktop, and jump boxes — the tools that let you reach a machine or network you're not physically sitting in front of.\n\nMost company systems live behind a VPN or require remote desktop into a specific box, so getting these configured on day one is what actually unblocks you from doing anything else. It's also worth knowing the difference between the two: a VPN gets your traffic *into* the network, remote desktop gets you *onto* a specific machine inside it.",
		el: 'VPN clients, remote desktop και jump boxes — τα εργαλεία που σου επιτρέπουν να φτάσεις σε ένα μηχάνημα ή δίκτυο στο οποίο δεν κάθεσαι φυσικά.\n\nΤα περισσότερα συστήματα της εταιρείας βρίσκονται πίσω από VPN ή απαιτούν remote desktop σε συγκεκριμένο μηχάνημα, οπότε η ρύθμισή τους από την πρώτη μέρα είναι αυτό που σε ξεμπλοκάρει για να κάνεις οτιδήποτε άλλο. Αξίζει να ξέρεις τη διαφορά: το VPN βάζει την κίνησή σου *μέσα* στο δίκτυο, το remote desktop σε βάζει *πάνω* σε ένα συγκεκριμένο μηχάνημα μέσα σε αυτό.',
	},
	Renewals: {
	...
	'Screen Sharing Tools': {
		en: "Tools that let you see (or take over) a customer's screen live, instead of trying to diagnose a problem through a written description alone.\n\nSome bugs are nearly impossible to describe in a ticket but obvious the second you actually see the screen — a wrong button clicked, a browser zoom level nobody mentioned. Knowing how to start a session quickly, and asking permission properly before you do, turns a 20-message back-and-forth into a two-minute call.",
		el: 'Εργαλεία που σου επιτρέπουν να δεις (ή να αναλάβεις) live την οθόνη ενός πελάτη, αντί να προσπαθείς να διαγνώσεις ένα πρόβλημα μόνο από γραπτή περιγραφή.\n\nΜερικά bugs είναι σχεδόν αδύνατο να περιγραφούν σε ένα ticket αλλά προφανή τη στιγμή που βλέπεις την οθόνη — ένα λάθος κουμπί, ένα zoom level του browser που κανείς δεν ανέφερε. Το να ξέρεις να ξεκινάς μια session γρήγορα, ζητώντας σωστά άδεια πρώτα, μετατρέπει 20 μηνύματα πάνω-κάτω σε μια κλήση δύο λεπτών.',
	},
	'Security & RBAC': {
